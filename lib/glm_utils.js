...
function nvr() { this.x = 0; this.y = 0; this.z = 0; }
function nfc() { this.n_v_e = 0; this.vert = [0, 0, 0, 0]; this.textCoordsIndex = [0, 0, 0, 0]; this.normalVertexIndex = [0, 0, 0, 0]; this.group = 0; this.material = 0; }
...
var glm = {
	loadOBJ: function (url, callback) {
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				var result = glmReadOBJ(xhr.responseText, mesh);
				mesh = result.mesh;

				if (result.fileMtl) {
					var mtlUrl = url.substring(0, url.lastIndexOf('/') + 1) + result.fileMtl;
					var xhrMtl = new XMLHttpRequest();
					xhrMtl.open('GET', mtlUrl, true);
					xhrMtl.onreadystatechange = function () {
						if (xhrMtl.readyState == 4) {
							if (xhrMtl.status == 200) {
								glmReadMTL(xhrMtl.responseText, mesh);
							}
							callback(mesh);
						}
					};
	...
	initMeshBuffers: function (gl, mesh) {
		var positions = [];
		var normals = [];
		var texcoords = [];
		var indices = [];
		...
		for (var i = 1; i <= mesh.nface; i++) {
			...
			for (var j = 0; j < face.n_v_e; j++) {
				var vi = face.vert[j];
				var ti = face.textCoordsIndex[j];
				var ni = face.normalVertexIndex[j];
				var key = vi + "_" + ti + "_" + ni;

				if (vertexIndexMap[key] !== undefined) {
					indices.push(vertexIndexMap[key]);
				} else {
					var v = mesh.vert[vi];
					positions.push(v.x, v.y, v.z);

					if (ni && mesh.normal[ni]) {
						normals.push(mesh.normal[ni].i, mesh.normal[ni].j, mesh.normal[ni].k);
					} else {
						normals.push(0, 1, 0);
					}

					if (ti && mesh.textCoords[ti]) {
						texcoords.push(mesh.textCoords[ti].u, mesh.textCoords[ti].v);
					} else {
						texcoords.push(0, 0);
					}

					vertexIndexMap[key] = nextIndex;
	...
	drawMesh: function (gl, mesh, program) {
